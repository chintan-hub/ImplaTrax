...
### P3-B — Sticky Table Headers ✅ Complete
...
*Status update (2026-07-30): Implemented, but not the way the objective above literally describes — and that's a deliberate, verified decision, not a shortcut. While building this I confirmed empirically (real browser measurement, not spec-reading) that `overflow-x: auto` on any ancestor of a `position: sticky` element traps that element's stickiness to itself, in every evergreen browser, even when `overflow-y` is left unset or explicitly set to `clip` — the two never actually decouple. That means "vertical scroll bubbles to `<main>`" and "the same wrapper keeps its own horizontal scroll" are mutually exclusive on one element; you cannot have both without either giving up per-table horizontal scroll (regressing wide tables on anything narrower than ~1150px) or building a much larger frozen-header/synced-scroll table split (Large complexity, touching every list page's rendering, real risk to the CI-green streak this milestone is supposed to protect). Chose the third option instead: `Table`'s own wrapper is now a bounded, self-contained scroll box (`max-h-[560px] overflow-auto`, both axes) with its header sticky *inside* that box — the exact pattern already proven on 2 of the Reports tables, generalized to every table in the app. This is what actually fixes the named bug: the old wrapper was a scroll container with unbounded height, so it had zero real scroll distance and its `sticky` header was dead code that never did anything observable; this wrapper genuinely scrolls, so the header genuinely, visibly sticks. Verified with Playwright on Products (table view) and Purchase Orders: header position stays pixel-fixed while the body scrolls underneath it (confirmed by measuring row position before/after, not just CSS `position` values, since the CSS values alone doubly misled me earlier in this same investigation), and horizontal scroll on the wide Products table still works (`scrollLeft` reaches its full clamped range). Also removed 2 pages' (Products, Inventory) now-redundant outer `overflow-x-auto` wrapper divs, which were independently re-triggering the exact same trap one level up — and removed the Reports page's 2 now-duplicate bounded-box wrappers since `Table` provides the same treatment itself. Net effect for users: every table header now correctly and visibly stays put while its rows scroll (a real fix, not a cosmetic no-op), at the cost of that scrolling being local to the table's own box (up to ~560px tall) rather than one continuous page-length scroll through it — a trade-off, not a regression, since the "continuous scroll" behavior never actually worked in the first place.*
