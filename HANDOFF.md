...
**P3-A Sticky Page Headers ✅** · **P3-B Sticky Table Headers ✅** (fixed the confirmed `Table.tsx` bug via a bounded self-contained scroll box per table rather than page-level bubbling — see `DEVELOPMENT_PLAN.md`'s status note for why) · **P3-C Responsive List Pages ✅** (column-priority hiding on Products/Purchase Orders/Cases/Loans) · **P3-D No-Wasted-Clicks Final Audit Pass ✅** (Inventory movement rows + Loan Returns now link to their PO/Sale/Loan/Case) · **P3-E Empty/Loading/Error State Consistency + root `ErrorBoundary` ✅** · **P3-F Search & Filter Consistency ✅** (`⌘K` now also covers Vendors/Sales/Loans/Purchase Orders, dead barcode-lookup placeholder removed, `Select`-vs-`Combobox` rule locked into `PROJECT.md` §2a and Inventory's one outlier fixed to match). Priority 4 is next — see below for its own open decision that must be resolved before P4-B/C can start.
