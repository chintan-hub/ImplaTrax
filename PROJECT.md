...
- **P1-I (Reports: new report types) shipped 2026-07-30.** Reports now has 9 tabs: the original Inventory/Sales/Loans/Purchases plus new Stock Valuation (per-SKU), Manufacturer-wise, Doctor-wise, and Batch/Lot + Expiry (both correctly gated on `clinicSettings.batchLotTrackingEnabled`, only appearing when it's on). All have CSV export. The next milestone is `DEVELOPMENT_PLAN.md` P1-J (print/export on every list page) — see `HANDOFF.md` §8.
