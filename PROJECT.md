...
8. **ImplantDesk must feel like professional desktop software, not a website.** Minimize clicks, scrolling, cursor travel, and repetitive actions. Keep frequently used controls visible (sticky search/filter/action bars where appropriate). Preserve user context — don't lose list scroll position, filters, or selection when navigating to a detail view and back. Support efficient bulk operations and keyboard shortcuts where practical. Prefer the interaction pattern a mature desktop application would use (Excel, File Explorer, Photoshop, AutoCAD, VS Code, Figma) over a generic web pattern when one applies more naturally. This is a standing license to refactor previously-built UI for usability (business logic must be preserved) — a local, low-risk friction fix should be made when it's found, not just noted. A larger change that needs to be consistent across many screens (e.g. a bulk-selection pattern, an app-wide keyboard-shortcut layer) belongs in `USABILITY_BACKLOG.md` at the repo root and should be implemented once it can be done consistently everywhere it applies, not piecemeal on one screen.
