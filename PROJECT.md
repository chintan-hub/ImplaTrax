...
- **P1-I and P1-J shipped 2026-07-30.** Reports now has 9 tabs (P1-I: the original 4 plus Stock Valuation, Manufacturer-wise, Doctor-wise, and Batch/Lot + Expiry, the latter two gated on `clinicSettings.batchLotTrackingEnabled`). Every list page (P1-J: Products, Inventory, Vendors, Patients, Cases, Labs, Users) now has a working "Export CSV" action that respects the page's current filters. "Print" was deliberately not added to list pages — see `DEVELOPMENT_PLAN.md`'s P1-J status note for the reasoning. The next milestone is `DEVELOPMENT_PLAN.md` P1-K (bulk Product import, sequenced last in Priority 1 as the highest data-integrity-risk item) — see `HANDOFF.md` §8.
