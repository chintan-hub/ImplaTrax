...
- **Filter widget rule (locked 2026-07-30, P3-F):** `Select` for a small, fixed, closed set of options (status, category, manufacturer, movement type) — the whole list is always short and never grows. `Combobox` for a filter over an open-ended, growing master-data list (a specific product, vendor, lab, doctor, or patient) — one where in-dropdown search meaningfully helps once there are more than a handful of entries. `⌘K` global search covers every major entity type a user might jump straight to (Products, Patients, Cases, Labs, Vendors, Sales, Loans, Purchase Orders) — a new entity type gaining its own detail page should gain its own search group at the same time, not later.
