...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-card border-border shadow-elevated transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300 flex flex-col',
  {
    variants: {
      side: {
        right:
  ...
  ({ side = 'right', className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <DialogPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 opacity-60 transition-opacity hover:opacity-100 hover:bg-surface-hover focus:outline-none focus:ring-2 focus:ring-ring">
          <X className="h-4 w-4" />
